...
function (spacepi_current_component_in_dir)
    file(RELATIVE_PATH relDir "${CMAKE_SOURCE_DIR}" "${ARGV2}")
    string(REGEX REPLACE "[/\\]" "_" compName "${relDir}")
    if ("${compName}" STREQUAL "")
        set(${ARGV0} "spacepi-${ARGV1}" PARENT_SCOPE)
    else()
        set(${ARGV0} "spacepi-${ARGV1}-${compName}" PARENT_SCOPE)
    endif()
endfunction()

function (spacepi_current_component)
    spacepi_current_component_in_dir(tmp "${ARGV1}" "${CMAKE_CURRENT_SOURCE_DIR}")
    set(${ARGV0} "${tmp}" PARENT_SCOPE)
endfunction()

function (spacepi_current_module)
    spacepi_current_component(tmp mod)
    set(${ARGV0} "${tmp}" PARENT_SCOPE)
endfunction()
...
function (spacepi_target_in_dir)
    spacepi_current_component_in_dir(tmp target "${ARGV1}")
    set(${ARGV0} "${tmp}" PARENT_SCOPE)
endfunction()

function (spacepi_current_target)
    spacepi_current_component(tmp target)
    set(${ARGV0} "${tmp}" PARENT_SCOPE)
endfunction()
...
function (spacepi_current_extension)
    spacepi_current_component(tmp ext)
    set(${ARGV0} "${tmp}" PARENT_SCOPE)
endfunction()
